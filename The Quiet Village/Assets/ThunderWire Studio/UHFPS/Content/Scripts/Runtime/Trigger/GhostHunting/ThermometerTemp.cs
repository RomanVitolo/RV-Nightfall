...
namespace UHFPS.Runtime
{
    public class ThermometerTemp : MonoBehaviour, ISaveable
    {
        ...
        private void Start()
        {
            // MULTIPLAYER PATCH: best effort only — in a networked session the player does not exist
            // yet, so EnsureThermometer() retries at each use site below.
            EnsureThermometer();
        }

        /// <summary>
        /// Resolves the thermometer from the local player's items once that player exists.
        /// </summary>
        /// <remarks>MULTIPLAYER PATCH. Netcode spawns the player after scene load.</remarks>
        private bool EnsureThermometer()
        {
            if (thermometer != null) return true;

            PlayerManager player = LocalPlayerContext.PlayerManager;
            ...
            if (TemperatureType == TempType.Base && !SaveGameManager.GameWillLoad)
