...
namespace UHFPS.Runtime
{
    ...
    public class GlareEffect : MonoBehaviour
    {
        ...
        private void Update()
        {
            // MULTIPLAYER PATCH: the player camera only exists once Netcode has spawned the local
            // player, which is after scene load. Nothing to glare toward until then.
            if (mainCamera == null)
            {
                var playerCamera = LocalPlayerContext.PlayerCamera;
                if (playerCamera == null) return;

                mainCamera = playerCamera.transform;
            }
