...
namespace UHFPS.Runtime
{
    ...
    public class CanvasLootAtCamera : MonoBehaviour
    {
        ...
        private void Update()
        {
            // MULTIPLAYER PATCH: the player camera appears only after Netcode spawns the local player,
            // so it is resolved on first use rather than cached in Awake.
            if (playerCamera == null)
            {
                playerCamera = LocalPlayerContext.PlayerCamera;
