...
namespace UHFPS.Runtime
{
    ...
    public class GameManager : MonoBehaviour
    {
        ...
        /// </summary>
        /// <remarks>
        /// MULTIPLAYER PATCH: built in the constructor rather than a field initializer. It used to read
        /// <c>Instance.GraphicReferencesRaw</c>; with the singleton gone the lambda has to close over
        /// this instance, and a field initializer cannot reference instance state.
        /// </remarks>
        public Lazy<IDictionary<string, Behaviour[]>> GraphicReferences { get; }

        public GameManager()
        {
            GraphicReferences = new Lazy<IDictionary<string, Behaviour[]>>(() =>
            {
                Dictionary<string, Behaviour[]> referencesDict = new();

                foreach (var reference in GraphicReferencesRaw)
                {
        ...
        // MULTIPLAYER PATCH: false until the networked local player exists and has been subscribed.
        private bool staminaSubscribed;

        /// <summary>
        /// Binds the stamina slider to the local player once that player exists.
        /// </summary>
        /// <remarks>
        /// MULTIPLAYER PATCH. Netcode spawns the player after scene load, so this cannot run in Awake
        /// any more. Returns false until the player is available, and Update retries.
        /// </remarks>
        private bool TrySubscribeStamina()
        {
            var playerManager = LocalPlayerContext.PlayerManager;
            if (playerManager == null) return false;

            var stateMachine = playerManager.PlayerStateMachine;
            if (stateMachine == null) return false;

            if (stateMachine.PlayerFeatures.EnableStamina)
            {
                stateMachine.Stamina.Subscribe(value =>
                {
        ...
        private void Awake()
        {
            ...
            // MULTIPLAYER PATCH: deferred — Netcode spawns the player after scene load, so there is no
            // state machine to subscribe to yet. Retried from Update until it succeeds.
            staminaSubscribed = TrySubscribeStamina();
        ...
        private void Update()
        {
            ...
            // MULTIPLAYER PATCH: retry the deferred subscription until the local player exists.
            if (!staminaSubscribed) staminaSubscribed = TrySubscribeStamina();

            // update stamina slider alpha
            if (staminaSubscribed && PlayerPresence.StateMachine.PlayerFeatures.EnableStamina)
            {
        ...
        public static void SubscribePauseEvent(Action<bool> onPaused)
        {
            ...
            if (gameManager == null)
            {
                // MULTIPLAYER PATCH: world objects subscribe from Awake, which runs before Netcode has
                // spawned the local player and its GameManager. Hold it and attach on bind.
                s_pendingPauseSubscribers.Add(onPaused);
                HookPendingFlush();
                return;
            }

            AttachPauseSubscriber(gameManager, onPaused);
        }
        ...
        public static void SubscribeInventoryEvent(Action<bool> onInventoryShown)
        {
            ...
            if (gameManager == null)
            {
                // MULTIPLAYER PATCH: see SubscribePauseEvent.
                s_pendingInventorySubscribers.Add(onInventoryShown);
                HookPendingFlush();
                return;
            }

            AttachInventorySubscriber(gameManager, onInventoryShown);
        }

        #region MULTIPLAYER PATCH: deferred subscriptions
        // GameManager is no longer a scene singleton; it arrives with the local player. Subscriptions
        // requested before that are queued here and attached once the player binds.
        private static readonly List<Action<bool>> s_pendingPauseSubscribers = new();
        private static readonly List<Action<bool>> s_pendingInventorySubscribers = new();
        private static bool s_pendingFlushHooked;

        private static void AttachPauseSubscriber(GameManager gameManager, Action<bool> onPaused)
        {
            var disposable = gameManager.IsPausedSubject.Subscribe(onPaused);
            gameManager.Disposables.Add(disposable);
        }

        private static void AttachInventorySubscriber(GameManager gameManager, Action<bool> onInventoryShown)
        {
            var disposable = gameManager.InventoryShownSubject.Subscribe(onInventoryShown);
            gameManager.Disposables.Add(disposable);
        }

        private static void HookPendingFlush()
        {
            if (s_pendingFlushHooked) return;

            s_pendingFlushHooked = true;
            LocalPlayerContext.Ready += FlushPendingSubscribers;
        }

        private static void FlushPendingSubscribers()
        {
            var gameManager = LocalPlayerContext.GameManager;
            if (gameManager == null) return;

            foreach (var subscriber in s_pendingPauseSubscribers)
            {
                AttachPauseSubscriber(gameManager, subscriber);
            }

            foreach (var subscriber in s_pendingInventorySubscribers)
            {
                AttachInventorySubscriber(gameManager, subscriber);
            }

            s_pendingPauseSubscribers.Clear();
            s_pendingInventorySubscribers.Clear();
        }
        #endregion
        ...
        {
            // MULTIPLAYER PATCH: returns default until the local player (and its GameManager) exists.
            var gameManager = LocalPlayerContext.GameManager;
            if (gameManager == null) return default;

            foreach (var module in gameManager.Modules.ManagerModules)
            {
