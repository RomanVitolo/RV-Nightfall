...
namespace QuietVillage.Multiplayer.Bridge.World
{
    ...
    public partial class WorldSync : NetworkBehaviour
    {
        ...
        public override void OnNetworkDespawn()
        {
            ...
            if (ReferenceEquals(QuietVillage.Multiplayer.Flow.SpawnGate.Active, this))
                QuietVillage.Multiplayer.Flow.SpawnGate.Active = null;

            if (ReferenceEquals(QuietVillage.Multiplayer.Characters.SavedCharacterSource.Active, this))
                QuietVillage.Multiplayer.Characters.SavedCharacterSource.Active = null;
        }
            ...
            foreach (var entity in FindObjectsByType<WorldSyncEntity>(FindObjectsInactive.Include))
            {
                ...
                if (string.IsNullOrEmpty(entity.Id))
                {
                    Debug.LogError($"{nameof(WorldSync)}: '{entity.name}' has no id and will not sync. " +
                                   "Re-run Tools > Quiet Village > Multiplayer > Set Up World Sync.", entity);
                    continue;
                }
                ...
                if (m_entities.TryGetValue(key, out var existing))
                {
                    Debug.LogError($"{nameof(WorldSync)}: '{entity.name}' collides with '{existing.name}'; only the first " +
                                   "will sync. Re-run Tools > Quiet Village > Multiplayer > Set Up World Sync.", entity);
                    continue;
                }
