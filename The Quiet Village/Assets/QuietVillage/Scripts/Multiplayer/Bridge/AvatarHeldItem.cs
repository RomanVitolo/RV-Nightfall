...
namespace QuietVillage.Multiplayer.Bridge
{
    ...
    public class AvatarHeldItem : MonoBehaviour
    {
        ...
        public void Bind(PlayerActionSync actions, Animator avatar, Inventory database)
        {
            ...
            if (m_models == null)
                Debug.LogWarning($"{nameof(AvatarHeldItem)}: no {nameof(AvatarHeldItemModels)} asset in Resources, " +
                                 ...
                                 "Run Tools > Quiet Village > Multiplayer > Set Up Held Item Models.", this);
        }
        ...
        private void Update()
        {
            ...
            // Empty-handed once dead: the item stays equipped on its owner's client, but a corpse holding it up reads as alive.
            var held = m_health != null && m_health.IsDead ? null : m_actions.EquippedItem;
