...
namespace QuietVillage.Multiplayer.Flow
{
    ...
    public class NetworkPlayerSpawner : MonoBehaviour
    {
        ...
        [Tooltip("Who each client chose to be. Without it everyone spawns as the default character.")]
        ...
        private void Spawn(ulong clientId, bool returning = false)
        {
            ...
            // Before spawning, so the character travels in the spawn and every client builds the right body at once.
            // Asked for now rather than when the level loaded: a resumed save's character is only known once this
            // client's account has reached the host, which is what a held spawn waits for.
