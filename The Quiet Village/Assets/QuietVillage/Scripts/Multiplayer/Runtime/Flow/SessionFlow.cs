...
namespace QuietVillage.Multiplayer.Flow
{
    ...
    public class SessionFlow : MonoBehaviour
    {
        ...
        private void Awake()
        {
            ...
            if (m_levels == null || m_levels.Default == null)
                Debug.LogError($"{nameof(SessionFlow)}: no {nameof(LevelCatalog)} with a level assigned, so no game can " +
                               "start. Run Tools > Quiet Village > Multiplayer > Set Up Lobby.", this);
        }
        ...
        {
            if (m_levels == null) return "No levels are set up. Run Tools > Quiet Village > Multiplayer > Set Up Lobby.";
            ...
            if (!Application.CanStreamedLevelBeLoaded(sceneName))
                return $"{DisplayNameOf(sceneName)} is not in Build Settings. Run Tools > Quiet Village > Levels > " +
