...
namespace Modules.Multiplayer.Bridge.Saves
{
    ...
    public static class MultiplayerSaveFile
    {
        ...
        /// <summary>
        /// A folder of its own per save, or the one folder the settings ask for.
        /// </summary>
        private static string NextFolderName(string savesPath, SerializationAsset settings, string scene)
        {
            ...
            if (!settings.SingleSave)
                return prefix + Directory.GetDirectories(savesPath, $"{prefix}*").Length.ToString("D3");

            return settings.UseSceneNames ? prefix + scene.Replace(" ", string.Empty) : prefix + "000";
        }
    }
}
