...
namespace Modules.Multiplayer.Bridge.EditorTools
{
    ...
    /// <c>Assets/Game/Prefabs/Drops</c> with physics added; the level's own pickups are untouched. The variant's asset GUID
    ...
    {
        private const string DropsFolder = "Assets/Game/Prefabs/Drops";
        private const string PlayerItemsSection = "Player Items";

        [MenuItem("Tools/Multiplayer/Set Up Droppable Items")]
        public static void Run()
        {
            ...
            if (database == null || references == null)
            {
                Debug.LogError($"{nameof(DroppableItemsSetup)}: the player prefab needs an Inventory with a database and a " +
                               "SaveGameManager with Object References. Run Tools > Multiplayer > Set Up Networked HEROPLAYER.");
                return;
            }
